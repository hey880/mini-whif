...
**최적화**:
- **토큰 캐싱**: 각 VU가 처음 한 번만 인증하여 Supabase rate limit 회피
- **랜덤 지터**: 인증 전 0-3초 랜덤 대기로 동시 인증 요청 분산
- **ConnectRPC 사용**: 채팅방 생성에 ConnectRPC 엔드포인트 사용
- 인증 후 1초 대기로 rate limit 완화

**효과**: 190개 VU가 10초에 걸쳐 추가될 때, 인증 요청이 0-13초에 분산되어 초당 약 15-20회로 제한

**주의**: Spike 테스트는 짧은 시간에 많은 요청을 보내므로 스테이징 환경 권장
